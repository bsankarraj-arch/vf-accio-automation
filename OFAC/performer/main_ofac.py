...
performer_path = os.path.join(BASE_DIR, "common", "ofac_perfomer.py")
...
# 3. Load the module
spec = importlib.util.spec_from_file_location("ofac_perfomer", performer_path)
ofac_mod = importlib.util.module_from_spec(spec)

# Add .venv to path so it can find auth_utils.py, etc.
sys.path.append(os.path.join(BASE_DIR, "common"))

spec.loader.exec_module(ofac_mod)

if __name__ == "__main__":
    performer = ofac_mod.OFACPerformer()
    performer.run()